"""
cape_in:
描述: CAPE 衡量大气中可用于对流的能量，是预测雷暴等强对流天气事件的重要指标。
高的 CAPE 值通常预示着较强的对流活动和潜在的恶劣天气条件。

tclw_in (Total Cloud Liquid Water):
描述: 总云水含量反映了特定区域云层中液态水的总量，对于预测降水事件和云的发展
非常重要。

sp_in (Surface Pressure):
描述: 表面气压是影响天气系统移动和强度的关键因素。气压的变化常常与天气变化紧
密相关，如气压低通常与风暴相关。

tcwv_in (Total Column Water Vapor):
描述: 总柱状水汽量是从地面到大气顶部的水汽总量，是评估大气中水分含量和潜在降
水的重要指标。

lsp_in (Large-Scale Precipitation):
描述: 大尺度降水通常与大气中的广泛运动系统（如锋面）相关，与广域的、持续时间
较长的降水事件有关。

cp_in (Convective Precipitation):
描述: 对流降水是由局部大气不稳定引发的，通常涉及短时间内的强降水，如雷雨和局
部暴雨。

tisr_in (Top of Atmosphere Incoming Shortwave Radiation):
描述: 大气顶部入射短波辐射是指太阳辐射到达地球大气层顶的能量，对于理解地球能
量平衡和气候系统至关重要。

u700_in 和 v700_in:
描述: 这两个变量分别表示700 hPa气压层的纬向和经向风速。在这个高度(约3000米)，
风速数据对于天气预报和了解大气中的水平气流非常重要。

yw_hourly_in (Hourly Precipitation, Model Output):
描述: 表示每小时的模型输出降水量，这通常是通过对更高分辨率的观测数据
（如 yw_hourly_tar）进行处理和降维以匹配模型或其他数据尺寸的结果。

yw_hourly_tar (Hourly Precipitation, Target Grid):
描述: 这代表特定目标地理网格的每小时雷达观测降水量，
其中“lat_tar”和“lon_tar”指的是目标纬度和经度。这通常用于模型验证和作为模型输出的基准。
...
class PrecipDatasetInter(torch.utils.data.IterableDataset):
    ...
    def __init__(self, file_path: str = None, 
        ...
        if local:
            ...
            self.vars_in_patches_list, self.vars_out_patches_list, self.vars_out_orig_patches_list, self.times_patches_list = self.process_netcdf(files)
    ...
    def process_netcdf(self, filenames: int = None, local=False):
        ...
        #input should shift from 7am tp 8am
        #inputs["tp"] = (['time', 'lat', 'lon'], tp)
        print("inputs 是什么样的呀好奇：",inputs)
        ...
        lats_tar = output_ori["lat_tar"].values
        lons_tar = output_ori["lon_tar"].values
        ...
        output_ori_nparray = output_ori.to_array(dim = "variables").squeeze().values.astype(np.float32)
        ...
        da_out_orig = torch.from_numpy(output_ori_nparray)

        self.n_samples = da_out.shape[0]

        del inputs_nparray, outputs_nparray,output_ori_nparray
        gc.collect()
        ...
        vars_out_orig_patches = da_out_orig.unfold(1, self.patch_size*self.sf, 
                                                   self.patch_size*self.sf).unfold(2,
                                                   self.patch_size*self.sf,self.patch_size*self.sf)

        vars_out_orig_patches_shape = list(vars_out_orig_patches.shape)
        vars_out_orig_patches = torch.reshape(vars_out_orig_patches,
                                          [vars_out_orig_patches_shape[0] * vars_out_orig_patches_shape[1] *
                                          vars_out_orig_patches_shape[2],
                                          vars_out_orig_patches_shape[3], 
                                          vars_out_orig_patches_shape[4]]) 
        ...
        vars_out_orig_patches = torch.index_select(vars_out_orig_patches, 0, no_nan_idx)
    ...
    def __iter__(self):
        ...
        for bidx in range(iter_start, iter_end):
            ...
            y_orig = torch.zeros(self.batch_size, self.patch_size*self.sf, self.patch_size*self.sf)
...
# 程序入口和测试运行
def run():
    data_loader = PrecipDatasetInter(file_path="/cpfs01/projects-HDD/cfff-4a8d9af84f66_HDD/public/ShiXiSheng/yzy/downscaling_precipitation/precip_dataset_new/train_small/")
