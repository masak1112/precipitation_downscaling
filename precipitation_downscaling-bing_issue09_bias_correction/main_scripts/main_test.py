...
def main():
    ...
    parser.add_argument("--k", type = float, default = 0.01, help = "The parameter for log-transform")
    ...
    if args.model_type == "diffusion" or args.model_type == "diffusion2" :
        ...
        with torch.no_grad():
            ...
            for i, test_data in enumerate(test_loader):
                ...
                # input_temp = (((np.squeeze(input_vars[:,-1,:,:])+1) / 2) * (vars_in_patches_max- vars_in_patches_min)+ vars_in_patches_min).cpu().numpy()
                input_temp = ((np.squeeze(input_vars[:,-1,:,:]) )* vars_in_patches_max).cpu().numpy()
                ...
                # preds = ((preds+1)/2) * (vars_out_patches_max - vars_out_patches_min) + vars_out_patches_min 
                preds = preds * vars_out_patches_max
                ...
                hr =  (model.hr.cpu().numpy()) * vars_out_patches_max
                ...
                # inter = ((model.L_inter.cpu().numpy()+1)/2) * (vars_in_patches_max - vars_in_patches_min) + vars_in_patches_min 
                inter = (model.L_inter.cpu().numpy()) * vars_in_patches_max
