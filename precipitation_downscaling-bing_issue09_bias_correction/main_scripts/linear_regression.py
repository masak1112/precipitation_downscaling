...
def main():
    ...
    parser.add_argument("--k", type = float, default = 0.02, help = "The parameter for log-transform")
        ...
        if len(noiseP.shape) == 4:
            ...
            with torch.no_grad():
                ...
                for i, test_data in enumerate(test_loader):
                    ...
                    hr = np.exp(hr+np.log(args.k))-args.k
                    hr_linear = hr[:].flatten()
                    preds_linear = preds[:].flatten()
                    # 使用最小二乘法拟合直线
                    A = np.vstack([preds_linear, np.ones(len(preds_linear))]).T
                    k, b = np.linalg.lstsq(A, hr_linear, rcond=None)[0]
                    K.append(k)
                    B.append(b)
    ...
    #  JSON 
    with open(f"{os.path.join(args.save_dir,'linear.json')}", 'w') as file:
        json.dump(linear_r, file)  # indent=4 用于美化格式
