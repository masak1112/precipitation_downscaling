...
def main():
    ...
    vars_in_patches_min = stat_data['in_min']
    vars_in_patches_max  = stat_data['in_max']
    vars_in_patches_avg = stat_data['in_avg']
    vars_in_patches_std  = stat_data['in_std']
    vars_out_patches_min = stat_data['out_min']
        ...
        if len(noiseP.shape) == 4:
            ...
            with torch.no_grad():
                ...
                for i, test_data in enumerate(test_loader):
                    ...
                    #input_temp = np.squeeze(input_vars[:,0,:,:])* (vars_in_patches_max - vars_in_patches_min )+ vars_in_patches_min 
                    input_temp = np.squeeze(input_vars[:,0,:,:])  #* (vars_in_patches_std )+ vars_in_patches_avg 
