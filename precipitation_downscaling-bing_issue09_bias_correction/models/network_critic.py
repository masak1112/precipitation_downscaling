...
class Conv_Block(nn.Module):
    ...
    def __init__(self, in_channels: int = None, out_channels: int = None,
                 kernel_size: int = 3, padding: str = "same", bias=True, stride=None, dropout_rate: float = 0.25):
        """
        The convolutional block consists of one convolutional layer, batch normalization, activation function and dropout.
        :param in_channels : the number of input channels
        :param out_channels: the number of output channels
        :param kernel_size : the kernel size
        :param padding     : the techniques for padding, either 'same' or 'valid' or integer
...
class Discriminator(torch.nn.Module):
    def __init__(self, shape, num_conv: int = 4, channels_start: int = 64, kernel: tuple = (3, 3),
                 stride: tuple = (2, 2), activation: str = "relu", lbatch_norm: bool = True, dropout_rate: float = 0.25):
        super().__init__()
        in_channels = shape[0]
        n_layers = nn.ModuleList([])

        out_channels = channels_start
        for _ in range(num_conv):
            n_layers.append(Conv_Block(in_channels, out_channels, kernel_size=kernel, stride=stride, dropout_rate=dropout_rate))
        ...
        self.flat = nn.Flatten()
        self.hidden = nn.Linear(512, channels_start)
        self.dropout_fc = nn.Dropout(p=dropout_rate)  # Dropout after hidden layer
        self.output = nn.Linear(64, 1)
    ...
    def forward(self, x):
        ...
        x = self.flat(x)
        x = self.hidden(x)
        x = self.dropout_fc(x)  # Apply dropout to hidden layer output
        return self.output(x)


# D = Discriminator((1,160,160))

# x = torch.rand((10,1,160,160))
